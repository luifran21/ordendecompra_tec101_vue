    ...
    <Teleport to="body">
        ...
        <div v-if="showModal" class="fixed left-0 top-0 bg-slate-50 rounded-xl w-10/12" style="
            ...
            <div class="w-full p-4 flex flex-col items-center justify-center">
                <p>¿Eliminar cliente con id "{{ producto.id }}" ({{ producto.nombre }} {{ producto.apellido }})?</p>
                <div class="grid grid-cols-2 gap-1">
                    <button class="bg-red-500 text-white p-1 rounded" @click="showModal = false">Cancelar</button>
                    <button class="bg-red-500 text-white p-1 rounded" @click="deleteProducto()">Eliminar</button>
                </div>
            </div>
        </div>
    </Teleport>
...
export default {
    ...
    methods: {
        deleteProducto() {
            axios.delete('/products/' + this.producto.id).then(
                (response) => {
